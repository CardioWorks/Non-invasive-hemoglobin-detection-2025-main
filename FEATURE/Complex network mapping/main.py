...
for i in range(4):
    for j in range(i, 4):
        ...
        similarity = np.linalg.norm(ppg_segments[i,:]- ppg_segments[j,:])
        # similarity = euclidean(ppg_segments[i,:], ppg_segments[j,:])
        # similarity, p_value = stats.pearsonr(ppg_segments[i,:],ppg_segments[j,:])
        W[i,j] = similarity
        W[j,i] = similarity
# G = nx.Graph(W)
# k = 0
